...
from functions.verifica_pastas import gerar_relatorio_pastas, caminhos_1
...
def carregar_df():
    return gerar_relatorio_pastas(caminhos_1) if LOCAL_ENV else pd.read_csv("atualizacao_dash_dados/csv/recepcao.csv")
...
for etapa in etapas_unicos:
    ...
    if LOCAL_ENV:
        caminhos_1_pasta = [caminhos_1.get(etapa, "") for et in df_etapa["Etapa"]]
        cache_atualizacao = {}
        for pasta in set(caminhos_1_pasta):
            if pasta and pasta in cache_atualizacao:
                cache_atualizacao[pasta] = ultima_data_arquivo(pasta)
        valores = [cache_atualizacao.get(p, "Indisponível") for p in caminhos_1_pasta if p]
